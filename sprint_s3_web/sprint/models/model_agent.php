...
function giveSold($nss){ //donne le solde en fonction du NSS
   $connexion=getConnect();
   $requete="SELECT solde_patient FROM patient WHERE nss='$nss'";
   $resultat=$connexion->query($requete);
   $resultat->setFetchMode(PDO::FETCH_OBJ);
...
// function givePrix($id_motif){ //Donne le prix d'un motif
//    $connexion=getConnect();
//    $requete="SELECT prix_motif FROM motif WHERE id_motif='$id_motif'";
//    $resultat=$connexion->query($requete);
//    $resultat->setFetchMode(PDO::FETCH_OBJ);
...
//    return $prix;
// }
// function giveMotif($id_motif){ //Donne du motif d'un rendez-voud
//    $connexion=getConnect();
//    $requete="SELECT id_motif FROM rdv WHERE id_motif='$id_motit'";
//    $resultat=$connexion->query($requete);
//    $resultat->setFetchMode(PDO::FETCH_OBJ);
...
function searchPatient($nss){ //cherche en fonction du NSS
   $connexion=getConnect();
   $requete="SELECT * FROM patient WHERE nss='$nss'";
   $resultat=$connexion->query($requete);
   $resultat->setFetchMode(PDO::FETCH_OBJ);
...
function changeStatut($nss,$id_motif,$id_employe,$date_creneau,$heure_creneau){
   $connexion=getConnect();
   $requete="UPDATE rdv SET statut_paiement='paye' WHERE nss='$nss' AND id_motif='$id_motif' AND id_employe='$id_employe' AND date_creneau='$date_creneau' AND heure_creneau='$heure_creneau'";
   $resultat=$connexion->query($requete);
   $resultat->closeCursor();
}
function searchRDV($nss){ //cherche en fonction du NSS
   $connexion=getConnect();
   $requete="SELECT * FROM (SELECT * FROM rdv WHERE nss='$nss') lesrdv NATURAL JOIN motif";
   $resultat=$connexion->query($requete);
   $resultat->setFetchMode(PDO::FETCH_OBJ);
...
function giveStatutPaiement($nss,$id_motif){
   $connexion=getConnect();
   $requete="SELECT statut_paiement FROM rdv WHERE nss='$nss' AND id_motif='$id_motif' AND statut_paiement='attente'";
   $resultat=$connexion->query($requete);
   $resultat->setFetchMode(PDO::FETCH_OBJ);
   $statut=$resultat->fetchall();
   $resultat->closeCursor();
   return $statut;
}
