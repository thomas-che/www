...
function displayListAttente($list){
   if ($list==null){
      $contents='<p>Aucun rendez-vous n\'a été programmé</p>';
   }else{
	  $i=1;
      foreach($list as $line){
		if($line->statut_paiement=="attente"){
			$contents.='<option>'.$i.' | '.$line->date_creneau.' | '.$line->heure_creneau.' | '.$line->prix_motif.' | </option>';
		}
		$i++;
	  }
...
function display1($c){
	$contents='<form method="post" action="clinique.php">
			<fieldset>
				<legend>Paiement du rendez-vous </legend>
			<p>
				<label for="nss"> Indiquer le NSS : </label>
				<input type="text" name="nss" id="nss"/>
			</p>
			...
			<p>
				<label for="nu">Numéro du rendez-vous choisi :</label>
				<input type="number" name="num" id="num"/>
			<p/>
				<label for="prix"> Prix à payer : </label>
